...
class ConsumerPool:
    ...
    def create_consumer(self, burst: bool = False):
        ...
        process = Process(target=run_consumer, name=name, kwargs=kwargs)
        process.start()
    ...
    def stop(self):
        ...
        for consumer in self._consumers.values():
            self.logger.debug(f"Ask consumer {consumer.name} to stop.")
            try:
                os.kill(consumer.pid, signal.SIGINT)
            except OSError as e:
                if e.errno == errno.ESRCH:
                    self.logger.debug(f"Consumer {consumer.name} already stopped.")
                else:
                    raise
