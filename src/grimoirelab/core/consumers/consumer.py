...
class Consumer:
    ...
    def __init__(
        ...
        self._stop = False
        self._requested_stop_date: datetime | None = None
    ...
    def process_entries(self, entries: Iterable[Entry]):
    ...
    def _force_stop(self, signum, frame):
        """Kill the consumer process"""

        # Running the consumer in a pool, sometimes receives duplicate signals.
        # One from the pool and another from the system. This check is to avoid
        # forcing the stop in that scenario.
        if (datetime.now() - self._requested_stop_date) < timedelta(seconds=1):
            return

        self.logger.warning("Forcing stop...")
    ...
    def _request_stop(self, signum, frame):
        ...
        self._requested_stop_date = datetime.now()

        signal.signal(signal.SIGINT, self._force_stop)
        signal.signal(signal.SIGTERM, self._force_stop)
