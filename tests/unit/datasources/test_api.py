...
class EcosystemDetailApiTest(APITestCase):
    ...
    def test_update_ecosystem_invalid_name(self):
        ...
        Ecosystem.objects.create(name="example-ecosystem", title="Example Ecosystem")
        url = reverse("ecosystem-detail", kwargs={"name": "example-ecosystem"})
        data = {"name": "123example-ecosystem"}
        response = self.client.patch(url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        self.assertEqual(response_json["name"], [INVALID_NAME_ERROR])
...
class ProjectListApiTest(APITestCase):
    ...
    def test_projects_parent_id_filter(self):
        ...
        project4 = Project.objects.create(
            name="project4", title="Project 4", parent_project=project3, ecosystem=ecosystem
        )
    ...
    def test_projects_term_filter(self):
        ...
        project1 = Project.objects.create(
            name="example-project1", title="Project 1", ecosystem=ecosystem
        )
        project2 = Project.objects.create(name="project2", title="Project 2", ecosystem=ecosystem)
        project3 = Project.objects.create(
            name="project3", title="Example project 3", parent_project=project2, ecosystem=ecosystem
        )
...
class ProjectDetailApiTest(APITestCase):
    ...
    def test_get_subproject(self):
        ...
        parent_project = Project.objects.create(
            name="parent-project", title="Parent project", ecosystem=ecosystem
        )
        Project.objects.create(
            name="example-project",
            title="Example Project",
    ...
    def test_update_parent_project(self):
        ...
        data = {"parent_project": parent_project.id}
        response = self.client.patch(url, data)
    ...
    def test_update_project_invalid_name(self):
        ...
        url = reverse(
            "projects-detail",
            kwargs={"ecosystem_name": "example-ecosystem", "name": "example-project"},
        )
        data = {"name": "123example-project"}
        response = self.client.patch(url, data)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        response_json = response.json()
        self.assertEqual(response_json["name"], [INVALID_NAME_ERROR])
...
class RepoListApiTest(APITestCase):
    ...
    def test_create_repository(self, mock_schedule_task):
        ...
        url = reverse(
            "repo-list", kwargs={"ecosystem_name": "ecosystem1", "project_name": "project1"}
        )
        data = {
            "uri": "https://example.com/repo.git",
            "datasource_type": "git",
    ...
    def test_create_repo_dataset(self, mock_schedule_task):
        ...
        repository = Repository.objects.create(
            uri="https://example.com/repo.git", datasource_type="git", uuid=uuid
        )
        url = reverse(
            "repo-list", kwargs={"ecosystem_name": "ecosystem1", "project_name": "project1"}
        )
    ...
    def test_add_dataset_already_exists(self):
        ...
        repository = Repository.objects.create(
            uri="https://example.com/repo.git", datasource_type="git"
        )
        DataSet.objects.create(
            project=self.project, repository=repository, category="commit", task=self.task
        )
        url = reverse(
            "repo-list", kwargs={"ecosystem_name": "ecosystem1", "project_name": "project1"}
        )
    ...
    def test_project_repo_list(self):
        ...
        DataSet.objects.create(project=self.project, repository=repository1, category="category3")

        repository2 = Repository.objects.create(
            uuid="BBB", uri="https://example.com/repo-2.git", datasource_type="git"
        )
        DataSet.objects.create(
            project=self.project,
        ...
        url = reverse(
            "repo-list", kwargs={"ecosystem_name": "ecosystem1", "project_name": self.project.name}
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 2)
    ...
    def test_project_repo_list_filter_category(self):
        ...
        DataSet.objects.create(project=self.project, repository=repository1, category="category2")
        repository2 = Repository.objects.create(
            uuid="BBB", uri="https://example.com/repo-2", datasource_type="github"
        )
        DataSet.objects.create(
            project=self.project,
            repository=repository2,
        ...
        repository3 = Repository.objects.create(
            uuid="CCC", uri="https://example.com/repo-3.git", datasource_type="git"
        )
        DataSet.objects.create(
            project=self.project,
...
class ProjectChildrenApiTest(APITestCase):
    ...
    def setUp(self):
        ...
        self.subproject1 = Project.objects.create(
            parent_project=self.project, name="subproject1", ecosystem=self.ecosystem
        )
        Project.objects.create(
            parent_project=self.subproject1, name="subproject2", ecosystem=self.ecosystem
        )
        ...
        DataSet.objects.create(
            project=self.project, repository=self.repository1, category="category1"
        )

        self.repository2 = Repository.objects.create(
            uri="https://example.com/repo2.git", datasource_type="git"
        )
        DataSet.objects.create(
            project=self.project, repository=self.repository2, category="category1"
        )
        DataSet.objects.create(
            project=self.subproject1, repository=self.repository2, category="category1"
        )
    ...
    def test_project_children_list(self):
        ...
        url = reverse(
            "children-list",
            kwargs={"ecosystem_name": "ecosystem1", "project_name": self.project.name},
        )
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 3)
